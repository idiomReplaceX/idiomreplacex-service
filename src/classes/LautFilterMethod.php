...
class LautFilterMethod implements IFilterMethod {
  ...
  protected function replacementTokens() {
    ...
    for ($i = 0; $i < $len; $i++){
      $char = $chrArray[$i];
      if(LautFilterMethod::$HTML_EL_END == $char && $isHtmlElement){
        // reaching end of HTML element
        $isHtmlElement = false;
        $tokenStartPos = $i + 1;
      } else if(LautFilterMethod::$HTML_EL_START == $char){
  ...
  protected function makeReplacementToken(array $chrArray, int $startPos, int $endPos): bool {
    ...
    if ($startPos > -1 && $startPos < $endPos) {
      ...
      if(preg_match("/[[:alnum:]]/", $tokenText )){
          if(strtolower($tokenText)=="laut") { 
            $this->rpTokens[] = new ReplaceToken($startPos, $tokenText, $this->replaceTokenText($tokenText));
            return true;
          }
      }
    }
  ...
  protected static function replaceTokenText($tokenText): string {
    ...
    return '<b style="font-size: 1.5em;">LAUT</b>';
  }
